...
- [ ] Scaffold `src-tauri/` (Cargo project + `tauri.conf.json`).
...
**Phase 1 log**
- Nothing implemented yet. The workflow currently skips when
  `src-tauri/tauri.conf.json` is absent.
...
- **2026-09-04 — Phase 0**
  ...
  - Added `ROADMAP.md`.
  - Added `.github/workflows/tauri-release.yml` (guarded until a Tauri config
    exists) and `.gitignore` entries for Rust/Tauri artifacts.
